...
<template>
  <view class="page-container h-screen flex flex-col overflow-hidden bg-[var(--page-bg)]">
    <wd-navbar title="添加餐食" placeholder left-arrow fixed @click-left="goBack">
      <template #right>
        <view class="save-btn from-emerald-500 to-emerald-600 bg-gradient-to-r" :class="{ disabled: !foodItems.length }" @click="handleSave">
    ...
    <scroll-view scroll-y class="flex-1 px-4 py-4 space-y-4">
      <view class="pb-24 space-y-4">
        <!-- 餐次提示 -->
        <view class="flex items-center gap-3 border border-emerald-100 rounded-xl bg-emerald-50 p-3 dark:border-emerald-800 dark:bg-emerald-900/20">
        ...
        <view v-if="foodItems.length" class="rounded-xl bg-[var(--card-bg)] p-4 shadow-sm">
          ...
          <view class="space-y-3">
            <view v-for="(item, index) in foodItems" :key="index" class="flex items-center justify-between rounded-lg bg-[var(--page-bg)] p-3">
              <view>
                ...
                <text class="text-[10px] text-[var(--text-sub)]">
                  {{ item.totalCalories }}kcal | P:{{ item.totalProtein }}g F:{{ item.totalFat }}g C:{{ item.totalCarbs }}g
                </text>
              </view>
        ...
        <!-- 汇总卡片 -->
        <view v-if="foodItems.length" class="border border-[var(--border-color)] rounded-xl bg-[var(--card-bg)] p-4 shadow-sm transition-all dark:border-[var(--wot-color-theme)]/10">
          ...
          <view class="grid grid-cols-2 gap-3">
            <view class="border border-[var(--border-color)] rounded-lg bg-[var(--page-bg)] p-3 shadow-sm">
              ...
              <view class="text-xl text-emerald-500 font-bold">
                {{ totals.calories.toFixed(0) }} <text class="ml-1 text-[10px] text-[var(--text-sub)] font-normal">
                  kcal
                </text>
              </view>
            </view>
            <view class="border border-[var(--border-color)] rounded-lg bg-[var(--page-bg)] p-3 shadow-sm">
              ...
              <view class="text-xl text-blue-500 font-bold dark:text-blue-400">
                {{ totals.protein.toFixed(1) }} <text class="ml-1 text-[10px] text-[var(--text-sub)] font-normal">
                  g
                </text>
              </view>
            </view>
            <view class="border border-[var(--border-color)] rounded-lg bg-[var(--page-bg)] p-3 shadow-sm">
              ...
              <view class="text-xl text-orange-500 font-bold dark:text-orange-400">
                {{ totals.fat.toFixed(1) }} <text class="ml-1 text-[10px] text-[var(--text-sub)] font-normal">
                  g
                </text>
              </view>
            </view>
            <view class="border border-[var(--border-color)] rounded-lg bg-[var(--page-bg)] p-3 shadow-sm">
              ...
              <view class="text-xl text-purple-500 font-bold dark:text-purple-400">
                {{ totals.carbs.toFixed(1) }} <text class="ml-1 text-[10px] text-[var(--text-sub)] font-normal">
                  g
                </text>
              </view>
            </view>
        ...
        <!-- 添加按钮 -->
        <view class="flex flex-col items-center justify-center gap-2 border-2 border-[var(--border-color)] rounded-xl border-dashed bg-[var(--card-bg)] p-6 transition-all active:border-emerald-500 active:bg-emerald-50" @click="goToFoodSelector">
          <view class="h-10 w-10 flex items-center justify-center rounded-full bg-emerald-50 text-emerald-500 dark:bg-emerald-900/20">
            <IconPlus size="20" />
          </view>
          <text class="text-sm text-emerald-600 font-bold">
            添加食物
