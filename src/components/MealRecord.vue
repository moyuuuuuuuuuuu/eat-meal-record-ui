...
<template>
  <view v-if="foods.length" class="mb-4 rounded-xl bg-[var(--card-bg)] p-4 shadow-sm">
    <view class="mb-3 flex items-center justify-between">
      <view class="text-sm text-[var(--text-main)] font-bold">
        {{ mealType }}
      </view>
    ...
    <view class="space-y-3">
      <view
        v-for="food in foods"
        :key="food.id"
        class="flex items-start justify-between border-l-4 border-emerald-500 rounded bg-[var(--page-bg)] py-2 pl-3"
      >
        <view class="flex-1">
